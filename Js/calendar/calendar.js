...
!function ($) {
    ...
    var weekdays = ['Domingo', 'Segunda', 'Terça', 'Quarta', 'Quinta',
        'Sexta', 'Sábado'];
    ...
    Calendar.prototype.scrollMonth = function (month) {
        ...
        if (month > 0) {
            ...
            this.render($prevMonth.length > 0 ? $prevMonth
                : $(this.$months[this.$months.length - 1]), true);
            this.$carousel.carousel('prev');
        }

        navegarBoletins(currentMonth, this.calendarDate.getFullYear());
    };
    ...
    Calendar.prototype.render = function ($element, navegar) {
        ...
        //carrega dias que tem boletim para exibir no calendário
        if (!navegar)
            carregarBoletins();
    };
        ...
        for (i = 1; i <= numDays; i++) {
            ...
            var todayClassName = (instance.today.getDate() == i
                && instance.getMonth() == instance.today.getMonth() && instance
                    .getYear() == instance.today.getFullYear()) ? '' : '';
...
function carregarBoletins(mes, ano) {
    ...
    ajaxProxy('WsSite', 'ListarBoletins', '{mes:' + mes + ',ano:' + ano + '}', function (result) {
        if (result.Assinante == "false") {
            $('#boletins').html('<ul class="list-inline list-unstyled text-center "><li><a href="/mercado/loja/assinaturas.aspx" title="Assinaturas"><img class="center-block" src="/mercado/images/banners/banner-exclusivo-assinante.png?v=2"/></a></li></ul>');
        }
        else {
            var html = '';
            ...
            for (var i in result.Items) {
                ...
                var dia = moment(result.Items[i].Data).date();

                $('#notes-calendar .month.active .day:contains("' + dia + '")').each(function () {
                    if (Number($(this).text()) == dia) {
                        ...
                        $(this).addClass('today');
                        $(this).html('<a href="/mercado/controls/downloadmembro.ashx?tipo=BoletimAcao&arquivo=' + dia + mes + ano + '.pdf" title="Baixar boletim">' + $(this).text() + '</a>');
                    }
                });
            }
...
function navegarBoletins(mes, ano) {
    ...
    ajaxProxy('WsSite', 'ListarBoletins', '{mes:' + mes + ',ano:' + ano + '}', function (result) {
        if (result.Error) {
            ...
            for (var i in result.Items) {
                ...
                $('#notes-calendar .month .day:contains("' + dia + '")').each(function () {
                    if (Number($(this).text()) == dia) {
